  ...
  return {
    ...
    execute: function () {
      ...
      _export("PlayerState", PlayerState = class PlayerState {
        constructor(_stateMachine, _animManage, _anim, _mode, _animName) {
          if (_mode === void 0) {
            _mode = AnimationClip.WrapMode.Normal;
          }
        ...
        createAnimClip() {
          ...
          return _asyncToGenerator(function* () {
            var spriteFrames = yield (_crd && ResourceManage === void 0 ? (_reportPossibleCrUseOfResourceManage({
              error: Error()
            }), ResourceManage) : ResourceManage).Instance.loadRes(_this.animPath);
            return new Promise((resolve, reject) => {
              _this.animClip = new AnimationClip(_this.animName);
              var frams = spriteFrames.map((item, index) => [_this.FRAME_TIME * index, item]);
              var track = new animation.ObjectTrack();
              track.path = new animation.TrackPath().toComponent(Sprite).toProperty('spriteFrame');
              track.channel.curve.assignSorted(frams); // 最后将轨道添加到动画剪辑以应用

              _this.animClip.addTrack(track);

              _this.animClip.duration = frams.length * _this.FRAME_TIME; // 整个动画剪辑的周期

              _this.animClip.wrapMode = _this.animMode;
              _this.animationManage.animComp.defaultClip = _this.animClip;

              _this.animationManage.animComp.play();
            });
          })();
        }
        ...
        enter() {
          ...
          this.animationManage.animComp.on(Animation.EventType.FINISHED, () => {
            var name = this.animationManage.animComp.defaultClip.name;
